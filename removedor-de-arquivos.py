...
def main()->None:
    ...
    logging.basicConfig(filename=f"Log - {timestr}.log",encoding='utf-8', level=logging.DEBUG, format='%(asctime)s %(levelname)s %(message)s', datefmt='%d/%m/%Y %H:%M:%S') #configura o formato de hora/data nas mensagens e a encodificação
    logger.info('Inicializando')

    with open('config.json') as config: #extraindo as informações de whitelist e destino do arquivo config.json da pasta do programa
        ...
        if os.path.exists(config_from_JSON["destino"]):  #Verifica se o endereço de destino dentro do JSON é um caminho válido no Sistema
            ...
            logger.info('Endereço %s válido', destino)
        else:
            logger.warning('Endereço Inválido. Por favor, verifique o arquivo config.json')
        ...
        if isinstance(config_from_JSON["whitelist"], list): #Verifica se a Whitelist é uma Lista Python válida (arrays são resolvidos como Listas quando são importados)
            ...
            if not whitelist: #Checa se a Whitelist está vazia
                logger.warning('Whitelist Vazia, isto removerá todos os arquivos da pasta de destino')
            else:
    ...
    arquivos: list[str] = [item for item in os.listdir(destino) if item not in whitelist]
    ...
    for nome_do_arquivo in arquivos:
        caminho_do_arquivo = os.path.join(destino, nome_do_arquivo)
        try:
            if os.path.isfile(caminho_do_arquivo): #Verifica se o arquivo atual é um arquivo
                os.remove(caminho_do_arquivo) #remove se for um arquivo
                logger.info("Arquivo %s removido", nome_do_arquivo)
            elif os.path.isdir(caminho_do_arquivo): #se não for um arquivo, verifica se é uma pasta
                shutil.rmtree(caminho_do_arquivo) #remove a pasta (e todos os arquivos de dentr)
                logger.info("Pasta %s removida", nome_do_arquivo)
        except OSError as e:
            logger.error(f"Falha ao remover {caminho_do_arquivo}. Erro: {e}") #Lança o erro no log, caso tenha algum erro
